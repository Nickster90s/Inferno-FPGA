...
//
//   1. boot with the filter OFF (the CSR resets to 0; this file never changes
//      that at init) and read nomatch_count -- that is a DRY RUN of exactly what
//      would be dropped, measured on hardware, before anything is at risk;
...
void rx_gate_init(const uint8_t mac[6])
{
    ...
    // Belt and braces: the CSR resets to 0, but say so out loud rather than
    // trusting a reset value across a bitstream change.
    rx_gate_enable_write(0);
    s_enabled = 0;
    s_pending = 0;

    printf("[rxgate] armed=0 local=%02x:%02x:%02x:%02x:%02x:%02x "
